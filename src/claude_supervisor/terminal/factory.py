...
def create_terminal(
    ...
    *,
    ...
    message pointing at the right extra.
    """
    if os.name == "nt":
        from claude_supervisor.terminal.backends import WinptyTerminal

        return WinptyTerminal(command, cwd=cwd, env=env)

    from claude_supervisor.terminal.backends import PexpectTerminal

    return PexpectTerminal(command, cwd=cwd, env=env)
...
def terminal_factory(
    *,
    ...
    """Return a :data:`TerminalFactory` bound to ``cwd``/``env``."""

    def factory(command: Sequence[str]) -> TerminalManager:
        return create_terminal(command, cwd=cwd, env=env)
